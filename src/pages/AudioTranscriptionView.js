    ...
    return (
        React.createElement(StaticPageView, { title: "النسخ الصوتي المباشر", icon: MicrophoneIcon },
            ...
            React.createElement("div", { className: "max-w-3xl mx-auto space-y-6 flex flex-col items-center" },
                React.createElement("div", { className: `w-full min-h-[200px] p-4 border-2 rounded-lg bg-light-100 dark:bg-dark-700 transition-colors ${isRecording ? 'border-primary' : 'border-light-300 dark:border-dark-600'}` },
                    transcript 
                ...
                React.createElement("p", { className: `text-sm font-semibold transition-colors ${isRecording ? 'text-primary animate-pulse' : 'text-dark-600 dark:text-dark-300'}` }, status)
            )
        )
    );
};