import React from 'react';

// المكون الوظيفي الذي يعرض الأيقونة الجديدة (Wallet/Security Pouch) مباشرةً
export const LockClosedIcon = ({
    className = "",
    size = 24, // الحجم الافتراضي
    color = "currentColor", // سيتم استخدام هذا اللون للتحكم في اللون الأساسي للأيقونة (Gray/Black)
    indicatorColor = "#69C229", // اللون الافتراضي للمؤشر الأخضر
    style = {},
    ...restProps
}) => {
    return (
        <svg
            version="1.1" 
            id="Layer_1" 
            xmlns="http://www.w3.org/2000/svg" 
            xmlnsXlink="http://www.w3.org/1999/xlink" 
            x="0px" 
            y="0px" 
            viewBox="0 0 122.88 101.13" 
            xmlSpace="preserve"
            width={size}
            height={size}
            className={className}
            style={{
                display: "inline-block",
                verticalAlign: "middle",
                // يمكن تحديد اللون الأساسي هنا عبر fill إذا كانت الأيقونة أحادية اللون
                ...style,
            }}
            {...restProps}
        >
            {/* تضمين الأنماط الداخلية للحفاظ على الألوان المتعددة */}
            <defs>
                <style type="text/css">
                    {`
                    /* نستخدم color الممرر في JSX للتحكم في اللون الأساسي st0 */
                    .st0{fill-rule:evenodd;clip-rule:evenodd;fill:${color};}
                    /* نستخدم indicatorColor الممرر للتحكم في لون المؤشر st1 */
                    .st1{fill:${indicatorColor};}
                    `}
                </style>
            </defs>
            
            <g>
                {/* المسار الأول (الجسم الرئيسي باللون الرمادي الداكن) */}
                <path className="st0" d="M86.64,45.01c3.23,0,6.1,1.56,7.89,3.96c1.79-2.41,4.66-3.96,7.89-3.96c5.43,0,9.83,4.4,9.83,9.83 s-4.4,9.83-9.83,9.83c-3.23,0-6.1-1.56-7.89-3.96c-1.79,2.41-4.66,3.96-7.89,3.96c-5.43,0-9.83-4.4-9.83-9.83 S81.21,45.01,86.64,45.01L86.64,45.01L86.64,45.01z M39.18,36.93c10.89,6.9,20.73,10.17,29.17,9.4c0.54,10.94-0.6,20.24-3.22,27.98 h51c0.78,0,1.49-0.32,2.01-0.84c0.52-0.52,0.84-1.23,0.84-2.01V33.99l0,0H3.91v37.47c0,0.78,0.32,1.49,0.84,2.01 c0.52,0.52,1.23,0.84,2.01,0.84h6.89c-2.58-7.74-3.82-17.15-3.53-28.44C20.03,46.4,29.76,44.25,39.18,36.93L39.18,36.93 L39.18,36.93z M63.63,78.22c-4.86,11.21-13.22,18.72-24.34,22.91C28.37,97.14,20.04,89.8,15.12,78.22H6.76 c-1.86,0-3.55-0.76-4.77-1.98C0.76,75.02,0,73.33,0,71.47V6.76C0,4.9,0.76,3.21,1.98,1.98C3.21,0.76,4.9,0,6.76,0l0,0h109.37 c1.86,0,3.55,0.76,4.77,1.98c1.22,1.22,1.98,2.92,1.98,4.77v64.7c0,1.86-0.76,3.55-1.98,4.77c-1.22,1.22-2.91,1.99-4.77,1.99H63.63 L63.63,78.22z M39.2,42.91c9.05,5.74,17.23,8.45,24.24,7.81c0.48,9.64-0.62,17.66-3.09,24.23l-0.07,0.18 c-0.08,0.2-0.15,0.4-0.23,0.59l-0.09,0.22c-0.07,0.18-0.15,0.36-0.22,0.54l-0.09,0.21c-0.08,0.18-0.16,0.36-0.24,0.54l-0.11,0.25 c-0.07,0.15-0.14,0.3-0.21,0.45l-0.15,0.3c-4.11,8.48-10.84,14.16-19.66,17.48c-8.68-3.17-15.39-8.71-19.54-17.48l-0.03-0.06 L19.7,78.1l-0.03-0.06c-0.07-0.16-0.14-0.31-0.21-0.47l-0.03-0.06l-0.03-0.06l-0.03-0.06l-0.03-0.06l-0.03-0.06 c-0.07-0.16-0.14-0.32-0.21-0.48l-0.03-0.06l-0.03-0.06l-0.03-0.06c-0.1-0.24-0.2-0.49-0.3-0.73l-0.02-0.06l-0.02-0.06 c-0.07-0.19-0.15-0.37-0.22-0.56l-0.02-0.06l-0.02-0.06l-0.02-0.06l-0.02-0.06c-0.07-0.19-0.14-0.38-0.21-0.57l-0.02-0.06 c-2.26-6.44-3.36-14.35-3.11-23.98C23.29,50.77,31.37,48.99,39.2,42.91L39.2,42.91L39.2,42.91z M3.91,14.56h115.06v-7.8 c0-0.78-0.32-1.49-0.84-2.01c-0.52-0.52-1.23-0.84-2.01-0.84H6.76c-0.78,0-1.49,0.32-2.01,0.84C4.23,5.27,3.91,5.98,3.91,6.76 V14.56L3.91,14.56L3.91,14.56z"/>
                
                {/* المسار الثاني (المؤشر الأخضر) */}
                <path className="st1" d="M24.62,67.34l5.17-0.07l0.39,0.1c1.04,0.6,2.03,1.29,2.95,2.07c0.66,0.56,1.3,1.17,1.9,1.84 c1.86-2.99,3.83-5.73,5.92-8.25c2.29-2.76,4.72-5.27,7.27-7.56l0.5-0.19h5.65l-1.14,1.26c-3.5,3.89-6.67,7.9-9.54,12.05 c-2.87,4.15-5.43,8.43-7.71,12.83l-0.71,1.37l-0.65-1.39c-1.2-2.58-2.64-4.95-4.36-7.07c-1.72-2.12-3.72-4-6.05-5.6L24.62,67.34 L24.62,67.34L24.62,67.34z"/>
            </g>
        </svg>
    );
};